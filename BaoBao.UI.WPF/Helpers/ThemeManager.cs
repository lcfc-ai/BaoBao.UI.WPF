...
namespace BaoBao.UI.WPF.Helpers
{
    ...
    public static class ThemeManager
    {
        ...
        public static void SetTheme(ThemeType theme)
        {
            ...
            // 2. 强制重新加载 Palette
            // 因为 Theme 文件可能会覆盖某�?DynamicResource 的查找路径，
            ...
            // 
            // 真正的问题可能是：切�?Palette 后，WPF 没有自动刷新那些使用�?DynamicResource 的控件？
            // 不，WPF DynamicResource 会监听字典变化�?
            // 
            // 让我们检�?UpdateDictionary 的逻辑�?
            // 当我�?SetPalette 时，只是替换�?Colors.xaml�?
            // 如果 Theme.Light.xaml 之前已经解析了颜色，WPF 需要知�?"BaoBao_Color_Primary" 变了�?
            // 替换 MergedDictionary 是正确的方法�?
            // 
            // 尝试一�?Trick：重新加�?Theme 也可以触发刷新�?
            // 但其实，保持 Palette �?Theme 之前可能更好�?
        ...
        private static void UpdateDictionary(string tag, string filePath)
        {
            ...
            if (oldDict != null)
            {
                // 如果�?Palette，我们需要它�?Theme 之前�?
                // 现有的逻辑�?Remove 然后 Add (添加到末�?�?
                // 如果 Theme 在列表里，Palette 被加到了 Theme 后面�?
                // 如果 Theme 用的�?StaticResource (之前是，现在我改成了 DynamicResource)，这会导致问题�?
                // 即便 DynamicResource，保持层级清晰也是好的： Palette (基础) -> Theme (映射) -> App资源
                
                // 让我们尝试原地替换，或者根�?Tag 决定插入位置�?
